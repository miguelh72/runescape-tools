...
import { ExpPage, Skill, Timeframe } from "../types";
import { internetingishardPage } from './results';
...
test('Load html page', async () => {
    const page = await crawler.__tests__.loadHtmlPage(internetingishardPage.url);
    expect(page.url).toEqual(internetingishardPage.url);
    expect(page.html.replace(/\s/g, '')).toEqual(internetingishardPage.html.replace(/\s/g, ''));
});
...
test('Retrieve all weekly exp gain pages for a skill', async () => {
    ...
    */
    ...
    const allPages: ExpPage[] = await crawler.getWeeklyExpPages(skill);

    function urlInPages(pages: ExpPage[], url: string): boolean {
        return pages.some(page => page.url === url);
    }

    //  all pages were returned or did not have enough players
    allUrls.forEach(url => {
        const wasProcessed = !persistence.fetchExpPage(url)?.hasData || urlInPages(allPages, url);
